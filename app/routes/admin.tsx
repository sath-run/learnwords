...
import { ValidatorError } from 'remix-validated-form/dist/types/validation/types';
...
const updatePasswordAction = async (userId: number, formData: FormData) => {
  ...
  const updateResult = await updatePassword(userId, result.data.currentPassword, result.data.password);
  if (!updateResult) {
...
const newAssignmentAction = async (userId: number, formData: FormData) => {
  ...
  await addAssignment(userId, name);
  return redirect(`/admin`);
};
...
export default function Admin() {
  ...
  useEffect(() => {
    ...
    if (
      ...
      ['actionRedirect', 'fetchActionRedirect'].includes(transition.type) && transition.location.pathname === '/admin'
    ) {
